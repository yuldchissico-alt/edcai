...
serve(async (req) => {
  ...
  try {
    ...
    console.log("Generating ad with body:", body);

    let systemPrompt: string;
    let userPrompt: string;

    if (prompt && typeof prompt === "string") {
      // Novo modo: prompt único descrevendo nicho, produto, objetivo etc.
      systemPrompt = `Você é um especialista em copywriting e anúncios digitais de alta conversão.
Seu objetivo é criar anúncios para redes sociais (TikTok, Reels, Meta Ads, etc.) a partir de um único prompt em linguagem natural.
...
REGRAS CRÍTICAS:
- Inferir nicho, público, plataforma e objetivo a partir do prompt do usuário
...
- CTA: ação clara + motivo de urgência`;
      ...
      userPrompt = `Prompt do usuário descrevendo produto, nicho, objetivo e público:
"""
${prompt}
"""

Use o prompt acima para criar o anúncio completo.
Retorne APENAS o JSON, sem texto adicional.`;
    } else {
      // Modo antigo: campos estruturados
      systemPrompt = `Você é um especialista em copywriting e anúncios digitais de alta conversão para ${platform}.
...
- Foque no benefício: ${mainBenefit}`;
      ...
      userPrompt = `Crie um anúncio para ${platform} no nicho de ${niche}.
    ...
    return new Response(
      JSON.stringify({
        error: error instanceof Error
          ? error.message
          : "Erro desconhecido ao gerar anúncio",
      }),
      {
        status: 500,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
